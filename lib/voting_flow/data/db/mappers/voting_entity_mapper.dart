...
class VotingEntityMapper extends DataMapper<VoteSlimDTO, InsertableSlimVoteModel> {
  final String? downloadedForDeputy;

  VotingEntityMapper({this.downloadedForDeputy});

  @override
  InsertableSlimVoteModel transform(VoteSlimDTO data) {
    final clubs = getClubMajority(data);
    return InsertableSlimVoteModel(
        entity: getVotingEntity(data), clubMajority: clubs, deputyVoteType: getClubNumberEntity(data));
  }

  VoteSlimEntityCompanion getVotingEntity(VoteSlimDTO data) => VoteSlimEntityCompanion.insert(
        id: data.id,
        date: data.voteAt,
        title: data.agenda,
        ...
        deputyVoteType: ValueExt.absentIfNull(data.deputyVoteType?.index),
        downloadedForDeputy: ValueExt.absentIfNull(downloadedForDeputy),
      );

  List<SlimClubMajorityEntityCompanion> getClubMajority(VoteSlimDTO data) => data.clubsMajority.expand((club) {
        ...
        return club.deputyCardNumbers.map((deputy) => SlimClubMajorityEntityCompanion.insert(
            votingId: data.id,
            parliamentClubId: club.parliamentClub,
            voteMajority: club.voteMajority.index,
  ...
  List<SlimDeputyVoteTypeEntityCompanion> getClubNumberEntity(VoteSlimDTO data) => data.deputiesVoteType
      .map((deputy) => SlimDeputyVoteTypeEntityCompanion.insert(
          votingId: data.id, deputyId: deputy.cadencyDeputy, type: deputy.voteType.index))
      .toList();
}
