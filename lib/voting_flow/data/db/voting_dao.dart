...
extension VotingDao on AthensDatabase {
  Future<void> insertVotingModel(InsertableSlimVoteModel model) async {
    await batch((batch) {
      batch.insert(voteSlimEntity, model.entity,
          mode: InsertMode.insertOrReplace);
      batch.insertAllOnConflictUpdate(
          slimClubMajorityEntity, model.clubMajority);
      batch.insertAllOnConflictUpdate(
          slimDeputyVoteTypeEntity, model.deputyVoteType);
    });
  }

  Future<void> insertVotingModelsList(
      List<InsertableSlimVoteModel> models) async {
    models.forEach((element) {
      insertVotingModel(element);
    });
  }

  Future<List<VoteSlimEntityData>> getVotings(int limit, int offset,
      Expression<bool?> Function($VoteSlimEntityTable tbl)? where) async {
  ...
  Future<void> voteWasSeen(String voteId) async {
    (update(voteSlimEntity)..where((tbl) => tbl.id.equals(voteId)))
        .write(VoteSlimEntityCompanion(viewed: Value(true)));
  }
  ...
  Future<List<SlimDeputyVoteTypeEntityData>> getClubNumbersForVoting(
      String votingId) {
