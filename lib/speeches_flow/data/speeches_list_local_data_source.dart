...
class SpeechesListLocalDataSource {
  ...
  SpeechesListLocalDataSource(this._database, this._toEntityMapper, this._toModelMapper, this._subscribedDeputiesCache);
  ...
  Future<List<SpeechModel>> getSpeechModels(int limit, int offset, SpeechesEasyFilter easyFilter) async {
    final entities = await _database.getSpeeches(limit, offset, [await _getEasyFilterWhere(easyFilter)]);
    return _toModelMapper(entities);
  }

  Future<Expression<bool?> Function($SpeechEntityTable)> _getEasyFilterWhere(SpeechesEasyFilter easyFilter) async {
    if (easyFilter is SpeechSeenFilter) {
      ...
      final Set<String> subscribedDeputiesIdSet =
          new Set.from(subscribedDeputies.map((deputy) => deputy.cadencyDeputyId));
      return ($SpeechEntityTable tbl) => tbl.deputyId.isIn(subscribedDeputiesIdSet);
