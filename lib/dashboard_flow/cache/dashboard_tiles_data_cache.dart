...
class DashboardTilesDataCache {
  ...
  DashboardTilesDataCache(this._getDashboardUseCase, this._mapper) {
    _autoRefreshSub = Rx.timer(null, Duration(minutes: 10))
        .listen((event) => markDirtyParam(DashboardParams.fetchAll(9)));

    Rx.combineLatest2(_dirtyParamsSource, _forceRefreshSource.startWith(null),
            (params, forceRefresh) => params as DashboardParams)
        .debounceTime(Duration(milliseconds: 500))
        .listen((DashboardParams dirtyParams) => _fetchDashboard(dirtyParams));
  }

  final BehaviorSubject<DashboardTilesDataModel?> _dashboardTilesDataSource =
      BehaviorSubject<DashboardTilesDataModel?>.seeded(null);
  final ReplaySubject<void> _forceRefreshSource = ReplaySubject<void>();

  Stream<DashboardNearestMeetingTileDataModel?> get nearestMeetingStream =>
      _dashboardTilesDataSource.map((event) => event?.nearestMeeting);
  Stream<DashboardSimpleDeputiesCounter?> get absentVoteStream =>
      _dashboardTilesDataSource.map((event) => event?.absentVote);
  Stream<DashboardSimpleDeputiesCounter?> get speechesCounterStream =>
      _dashboardTilesDataSource.map((event) => event?.speechesCounter);
  Stream<DashboardSimpleDeputiesCounterPerYearDataModel?>
      get absentVotePerYearStream =>
          _dashboardTilesDataSource.map((event) => event?.absentVotePerYear);
  Stream<DashboardSimpleDeputiesCounterPerYearDataModel?>
      get speechesCounterPerYearStream => _dashboardTilesDataSource
          .map((event) => event?.speechesCounterPerYear);

  late StreamSubscription _autoRefreshSub;

  final BehaviorSubject<DashboardParams> _dirtyParamsSource =
      BehaviorSubject<DashboardParams>.seeded(DashboardParams(
          cadence: 9,
          meeting: true,
          monthMeetings: true,
  ...
  markDirtyParam(DashboardParams dirtyParams) {
    ...
    if (dirtyParams.monthMeetings == true)
      currentDirtyParams.monthMeetings = true;
    if (dirtyParams.speechesCounter == true)
      currentDirtyParams.speechesCounter = true;
    if (dirtyParams.speechesCounterPerYear == true)
      currentDirtyParams.speechesCounterPerYear = true;
    if (dirtyParams.voteAbsent == true) currentDirtyParams.voteAbsent = true;
    if (dirtyParams.voteAbsentPerYear == true)
      currentDirtyParams.voteAbsentPerYear = true;
  ...
  void _fetchDashboard(DashboardParams dirtyParams) async {
    ...
    if (result.isSuccess()) {
      ...
      final dashboardTilesData =
          await _mapper.transform(result.toSuccess().value);
      _dashboardTilesDataSource.add(dashboardTilesData);

      _dirtyParamsSource.add(DashboardParams.allPristine(cadence: 9));
    } else if (result.isFailure()) {
      _fetchingFinished.addError((result as Failure).exception);
    }
  }
